...
Яркий представитель подобных контейнеров с `contiguous` итератором: `std::span`, который содержит лишь указатель на начало блока и его размер, что делает его легковестным и `C ap` совместимым
