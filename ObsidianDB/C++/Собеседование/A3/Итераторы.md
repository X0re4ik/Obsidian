...
false
```
2) `UB` не будет, потому что в станлартной реализации `std::vector` при `size == capacity` происходит релокация памяти, а у нас `size = 3; capacity = 4` - есть запас в 1 объект. 