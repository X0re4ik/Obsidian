...
`std::vector` - базовый контейнер, который входит в стандартную библиотеку `std`. 
...
Суммарно пустой вектор всегда занимает 24 байта памяти (`begin_` + `end_` + `end_of_storage_`). Аллокатор - логически часть `std::vector`; физически может занимать 0 байт для пустого аллокатора благодаря оптимизациям компилятора.
