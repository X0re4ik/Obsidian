...
`std::vector` содержит обязательный набор полей, упрощенно их можно представить так (Сгенерировано через Perplexity):
...
`std::vector` - **динмаический массив**, значит он способен к расширению, а значит 
