...
1) Создание маасива под 10 элементов
```C++
std::vector<int> v;
v.reserve(10);
```
...
```
4) Создание массива из указателя:
```C++
int array[] = {10, 20, 30, 40, 50};
std::vector<int> v(array, array + 5);
// v == {10, 20, 30, 40, 50}
```
5) Создание указателя из итератора:
```C++
...
При создании массива из итератора руководствуемся правилом: `[fisrt, last)` - то есть конец итератора не в
