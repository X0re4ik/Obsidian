...
Типичная реализация `std::vector` содержит указатель на начало выделенной памяти, позицию после создания последнего элемента, , упрощенно их можно представить так (Сгенерировано через Perplexity):
...
Суммарно пустой вектор всегда 24 байта **памяти на стеке** (`begin_` + `end_` + `end_of_storage_`). Аллокатор - логически часть `std::vector`; физически может занимать 0 байт для пустого аллокатора благодаря оптимизациям компилятора.
