...
Если пользователь запрашивает вставку 6 элмента при `capacity = 5`, то массив будет вынужден выполнить расширения `capacity`, чаще всего (в `libstdc++` и `libc++`) путём `capacity = capacity * 2`. Поэтому размер `capacity` "прыгает": 1  
