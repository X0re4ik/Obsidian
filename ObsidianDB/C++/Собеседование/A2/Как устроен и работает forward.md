...
По сравнению с реализацией `std::move` ([[Как устроен и работает move#^829aaa]]) видны несколько значимых отличий:
...
Теперь рассмотрим, что возвращает `std::forward` с учётом правила свёртывания ссылок ([[Как устроен и работает move#^7dc08c]]]).
