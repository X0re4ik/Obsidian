...
`explicit` применяется к конструкторам с C++98, в том числе с несколькими параметрами. До C++11 неявные преобразования через конструктор требовали возможности вызова с одним аргументом. C++11 добавил списковую инициализацию и применение `explicit` к операторам преобразования.
...
В C++11 появилась списковая инициализация: `explicit` запрещает использовать конструктор при копирующей списковой инициализации, например `SimpleExplicit s = {11, 12}`. Прямая форма `SimpleExplicit s{11, 12}` разрешена.
