...
Опытным `C++` разработчикам эта проблема известна, как [`ODR` (`One Definition Rule`)](https://en.cppreference.com/cpp/language/definition). Но корень этой ошибки выходит далеко за рамки "используй inline в `.hpp` файлах " и "не определяй ничего в `.hpp`"

В этой небольшой статье мы попробуем разобраться, как наши объектные файлы, которые мы создаем видит линковщик и другие единицы трансляции. 
