...
Волшебное ключевое слово `static` сделало все наши переменные внутренними, теперь всякий раз, когда мы выполняем `#include "bar.hpp"`  -  произойдет копирование наших переменных в новую единицу трансляции, переменные будут локальными и для *каждой* единице трансляции *свои* (Очень классно описана боль работы со )
