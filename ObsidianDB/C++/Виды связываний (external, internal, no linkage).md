...
Мы уже знаем, что `external_bar` и `external_foo` являются внешними и будут нам доступны из основной программы, `foo` так же является внешним, так как стандарт диктует, что все объявления внутри класса получают inline (то есть `external linkage`), но результат совершенно не ясен. Ответ кроется в то, *как* мы скомпилируем нашу программу
