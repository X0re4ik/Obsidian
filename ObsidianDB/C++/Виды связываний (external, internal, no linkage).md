...
Естественно, любая подобная тема по `C++` не могла обойтись без примера шаблонного метода или функции. На самом деле стандарт уже дал ответ на решение проблемы `ODR` для шаблонных функций, поэтому компилятор все шаблонные функции и методы помечает, как `wea`

Это справедливо и к шаблонным функциям и классам, так как они требуют определения на месте (в общем случае), то так же получают неявный `inline`, который спасает их от `ODR`
