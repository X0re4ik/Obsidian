...
Мы уже знаем, что `external_bar` и `external_foo` являются внешними и будут нам доступны из основной программы, `foo` так же является внешним, а вот  вопрос не ясен, по итогу наша программа любит оливки, или предпочитает нечто иное...
