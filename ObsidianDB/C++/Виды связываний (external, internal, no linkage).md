...
Особое внимание на 2 второй столбец, в процессе изучения темы мы получим 3 возможных варианта: `l` (`local`), `g` (`global`), `w` (`weak`) - именно эти значения определяют то, как линковщик будет "дружить" наш объектный файл с такими коллегами
...
По умолчанию все *глобальные переменные* и *объявления функций* имеют внешнее связывание (`external linkage`). Внешнее связывание потенциально несет большую проблему: если при линковки линковщик обнаружит 2 идентичных символа с типом `global` - ошибка множественного определения, поэтому любое объявление внутри `.hpp` - потенциальная ошибка `ODR`.
...
Особенную роль занимают классы, enum-ы и typedef - формально, они имеют внешнее связывание, но `objdump` ничего по ним не покажет, так как это абстракция языка, о которой наши объектные файлы не очень хотят и знать. Но о классах и специфики их работы м
...
Классы, структуры, перечисления, `typedef` и `using` не имеют связывания. Так как они не являются переменными или функциями, а лишь описывают типы данных - те самые абстракции, о которых наш компуктер не хочет даже слышать. Они существуют только на этапе компиляции и не порождают никаких символов в объектном файле, поэтому линковщику нечего с ними делать.
