...
Мы видим, что по умолчанию `foo` была `external` со слабым типом связанности и имела слабую связанность, но аннонимность позволила превратить `foo` в `internal`, что позволило избежать `ODR`, а так же наша позволило нашей программе наконец-то твердо и чётко заявить о своем отношении к оливкам
```bash
>> g++ main.cpp bar.cpp foo.cpp -o main
>> ./main
Я люблю оливки
