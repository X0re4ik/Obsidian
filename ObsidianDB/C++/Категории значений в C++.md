...
* Указатель `this`
Свойства `prvalue`:
* `prvalue` - это чистое знаачение, у него нет идентичности объекта, нет адреса, нет полимрфного поведения. Он либо вычисляется, либо материализуется в объект строго того типа, что записан в выражении
* `prvalue` не может быть полиморфным, потому что для полиморфизма необходима ссылка или указатель - `lvalue`


Полный список `xvalue` значений перечислен [здесь](https://ru.cppreference.com/cpp/language/value_category), перечислим лишь наиболее типичные:
